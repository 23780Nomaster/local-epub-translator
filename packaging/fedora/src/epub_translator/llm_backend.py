...
class TranslationServer:
    def __init__(self, model_path: str, llama_bin_dir: str = None):
        ...
        if llama_bin_dir is None:
            llama_bin_dir = os.path.join(os.path.expanduser("~"), ".local", "llama-cpp")
        self.llama_bin_dir = os.path.expanduser(llama_bin_dir)
        self.port = _find_free_port()
        self._proc = None

    def start(self):
        server_bin = os.path.join(self.llama_bin_dir, _server_binary_name())
        env = os.environ.copy()

        if IS_WINDOWS:
            env["PATH"] = self.llama_bin_dir + os.pathsep + env.get("PATH", "")
        else:
            env["LD_LIBRARY_PATH"] = self.llama_bin_dir + ":" + env.get("LD_LIBRARY_PATH", "")
